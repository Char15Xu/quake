...
Each partition is uploaded as a separate object with key:
    {prefix}/partition_{pid}

The object body is raw binary: codes (nv * code_size bytes) followed by
ids (nv * 8 bytes), matching what DynamicInvertedLists::s3_fetch_partition()
...
def upload_index_to_s3(index_dir, bucket, prefix,
    ...
    Partition data (each partition as a separate S3 object) is uploaded from
    the binary partitions file.  The metadata.txt file and parent/ directory
    are uploaded as regular files so the local index directory can be used
    together with S3 partition data.
